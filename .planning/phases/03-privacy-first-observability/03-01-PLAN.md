...
- configure structlog with contextvars support, including structlog.contextvars.merge_contextvars in the processor chain so bound correlation_id is emitted in JSON output.
- include timestamp, level, and JSON renderer (pretty when enabled).
...
Do not log any payloads here; this module only configures logging.
  ...
  <done>Observability dependencies are listed, logging configuration initializes when enabled, and bound contextvars (e.g., correlation_id) appear in JSON log output.</done>
</task>
